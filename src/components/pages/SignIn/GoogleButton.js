...
const GoogleLoginButton = () => {
  ...
  return (
      <button className="
        flex justify-center items-center w-[50%] h-[2.5rem] bg-layout-second text-white
        rounded-lg shadow-md px-6 py-2 text-sm font-medium transition-all
        hover:bg-primary/50"
        type='button'
        onClick={() => {
          if(!isProcessSignInWithPlatform) {
            dispatch(actionAuth.processSignInWithGoogle())
          }
        }}
      >
        {
          isProcessSignInWithPlatform ? <BarLoader 
            color="#eb4335"
          /> : <>
          <svg
